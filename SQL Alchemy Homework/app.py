...
def prcp_analysis():
    ...
    result = session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >= last12mos).all()
    prcp_dict = {}
    for data in result: 
...
def tobs_analysis():
    ...
    result = session.query(Measurement.date, Measurement.tobs).filter(Measurement.date >= tobs12mos).all()
    tobs_dict = {}
    for data in result: 
